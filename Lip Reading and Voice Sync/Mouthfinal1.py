        ...
        for (x,y,w,h) in faces:
            try:
                ...
                for(mx,my,mw,mh) in mouth:
                    ...
                    graysc = cv2.cvtColor(resize_m, cv2.COLOR_BGR2GRAY)
                    #Working partially.. imgthreshold=cv2.inRange(resize_m,cv2.cv.Scalar(106,93,201),cv2.cv.Scalar(173,193,238),)
                    imgthreshold=cv2.inRange(graysc,cv2.cv.Scalar(133,21,199),cv2.cv.Scalar(203,193,255),)
                    im2, hierarchy = cv2.findContours(imgthreshold,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
                    #edges=cv2.Canny(imgthreshold,100,200)
                    #cv2.imshow('Filters',edges)
                    areas = [cv2.contourArea(c) for c in im2]
                    max_index = np.argmax(areas)
                    cnt=im2[max_index]
                    x,y,w,h = cv2.boundingRect(cnt)
                    cv2.rectangle(resize_m,(x,y),(x+w,y+h),(255,0,0),2)
                    cv2.drawContours(resize_m,cnt,-1,(0,255,0),4)
                    cv2.imshow('Filter',resize_m)
                    cv2.imwrite( pathdir+nome+'/'+str(count2)+'.jpg', resize_m );
                    count2+=1
                    #lower_pink = np.array([110,50,50])
                    #upper_pink = np.array([130,255,255])1
                    #mask = cv2.inRange(hsv, lower_pink, upper_pink)
                    #res = cv2.bitwise_and(resize_m,resize_m, mask= mask)
                    
                    #cv2.imshow('threshold',graysc)
